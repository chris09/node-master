...
var profile = yaml.safeLoad(fs.readFileSync("./profile.yml", "utf-8"));
...
fs.readFile("./profile.yml", "utf-8", function (err, file) {
	var profile = yaml.safeLoad(file);
	var message = profile.name + " is " + profile.age + " years old and likes ";
	message += profile.movies.join(" and ") + ".";

	var filepath = path.join(".", "profiles", "mel-jones.txt");
	fs.writeFile(filepath, message, "utf-8", function(err){
		if (err){
			console.error(err);
			return;
		}

		console.log("Profile for " + profile.name + " writtten to " + filepath);
	});
});
